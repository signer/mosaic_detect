...
def ont(th):
	cv.CvtColor(iDiff, color, cv.CV_GRAY2BGR)
	mb = [0]*(w*h)
	# h*w macroblocks
	...
	for i in range(h):
		for j in range(w):
			...
			vleft = 0
			for k in range(16):
				vtop += (iDiff[top, left+k]>th)
			for k in range(16):	
				vleft += (iDiff[top+k, left]>th)
			if (vtop >= 15):
				cv.Line(color, (left, top), (left+15, top), cv.CV_RGB(255, 0, 0), 
						1, 4)
				mb[i*w+j] += 1
				if (i>0): mb[(i-1)*w+j] += 1
			if (vleft >= 15):
				cv.Line(color, (left, top), (left, top+15), cv.CV_RGB(255, 0, 0), 
						1, 4)
				mb[i*w+j] += 1
				if (j>0): mb[i*w+j-1] += 1
			cv.Line(color, (left, top), (left, top), cv.CV_RGB(255, 0, 0), 1, 4)
	...
	for i in range(h):
		for j in range(w):
			if (mb[i*w+j] >= 3):
				...
				for k in range(15):
					for l in range(15):
						v += iDiff[top+k, left+l]
				v = int(v/225)
				cv.PutText(color, str(v), (left+1, top+12), font, 
						cv.CV_RGB(0, 255, 0))
	cv.ShowImage('diff', color)
	return 
